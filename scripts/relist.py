...
def scan_map(
    ...
    segment_matcher = re.compile(
        "^([A-Z][A-Z0-9_]*) *([0-9A-F]{6})  ([0-9A-F]{6})  ([0-9A-F]{6})  ([0-9A-F]{5})$",
        re.IGNORECASE,
    )
    lst_matcher = re.compile(
        "^    ([A-Z][A-Z0-9_]*) *Offs=([0-9A-F]{6})  Size=([0-9A-F]{6})  Align=([0-9A-F]{5})  Fill=([0-9A-F]{4})$",
        re.IGNORECASE,
    )
    obj_filename = f"{os.path.basename(os.path.splitext(lst_filename)[0])}.o:"

    logger.debug(f"Looking for {os.path.basename(os.path.splitext(lst_filename)[0])}.o")
    ...
    with open(map_filename) as f:
        ...
        for line in f:
            ...
            if line.rstrip() == obj_filename:
                ...
                logger.debug(f"Found module {obj_filename} on line {line_num}")
                ...
                logger.debug(f"Found segment list on line {line_num}")
                reading_segments = True
            elif reading_segments:
                if line.rstrip() == "":
                    logger.debug(f"Segment list finished on line {line_num}")
                    reading_segments = False
        ...
        logger.debug(f"Done reading {line_num} lines")
...
def scan_lst(lst_filename) -> str:
    ...
    with open(lst_filename) as f:
        ...
        for line in f:
            ...
            if (
                not segment_match
            ):  # Wasn't .segment, check one of the special segment names
                segment_match = spc_matcher.match(code_part)
            if segment_match:
                if cur_segment is not None:
                    logger.debug(
                        f"{line_num}: Creating new segment {cur_segment} with size ${cur_size:04X}"
                    )
                    module.add_segment(cur_segment, 0, cur_size)
                cur_segment = segment_match.group(1).upper()
                cur_size = 0
                logger.debug(f"Scanning new segment {cur_segment}")
            else:
                ...
                if lst_match:
                    ...
                    logger.debug(
                        f"{line_num}: new_size = {new_size} + {len(ops)} ?> {cur_size}"
                    )
                    if new_size > cur_size:
                        logger.debug(
                            f"{line_num}: new_offset {new_size:X}, old_offset {cur_size:X}"
                        )
                        cur_size = new_size
        if cur_segment is not None and cur_size != 0:
            logger.debug(
                f"{line_num}: Creating new segment {cur_segment} with size ${cur_size:04X}"
            )
            module.add_segment(cur_segment, 0, cur_size)
    logger.debug(f"Done scanning {lst_filename}")
...
def process(map_filename, lst_filename):
    logger.debug(f"Processing {lst_filename}")
    ...
    if module_name not in map_modules:
        logger.info(
            f"INFO: unique module name {module_name} did not appear map file modules. We're going to wing it..."
        )
        ...
        if module_name == None:  # we still didn't find anything...
            logger.warn(
                f"WARNING: we couldn't even find a map file module named {bare_module_name}"
            )
            global warning_flag
            warning_flag = True
        else:
            logger.info(f"INFO: we're going to go with {module_name}.")
    ...
    for name, segment in module.segments.items():
        logger.debug(
            f"segment name = {segment.name}, size = {segment.size}, offset = {segment.offset:X} + {map_segments[name].offset:X}"
        )
    ...
    with open(rlst_filename, "w") as rlst_file:
        logger.debug(f"Writing relisting to {rlst_filename}")
...
if __name__ == "__main__":
    ...
    if lst_path.endswith(".lst"):
        ...
        logger.debug(f"glob pattern = {glob_pattern}")
        all_lst = glob.glob(glob_pattern, recursive=True)
    if len(all_lst) == 0:
        warning_flag = True
        logger.warn(f"WARNING: No lst file(s) to process.")
    for lst in all_lst:
